...
export default function SettingsPage() {
  ...
  // --- NOVO: APLICA AS CONFIGURAÇÕES VISUAIS ---
  useEffect(() => {
    if (loading) return;

    const root = document.documentElement;
    const { theme, primary_color, compact_mode } = settings.appearance;

    // 1. Aplicar Tema (Dark/Light)
    ...
    // 2. Aplicar Cor Primária (CSS Variable)
    // Isso injeta a cor na raiz para ser usada globalmente
    root.style.setProperty('--primary-color', primary_color);

    // 3. Aplicar Modo Compacto
                  ...
                  <div key={item.key} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input 
                        ...
                        onChange={() => toggleBooleanSetting('notifications', item.key as any)}
